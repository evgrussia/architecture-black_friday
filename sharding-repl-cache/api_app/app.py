...
async def _get_shard_documents_count(collection_name: str) -> Optional[dict]:
    """Для шардированного кластера возвращает количество документов по каждому шарду (через $collStats)."""
    try:
        collection = db.get_collection(collection_name)
        cursor = collection.aggregate([{"$collStats": {"count": {}}}])
        result = {}
        async for doc in cursor:
            shard = doc.get("shard") or doc.get("host")
            count = doc.get("count", 0)
            if shard is not None:
                result[str(shard)] = count
...
async def root():
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", []):
            shard_id = shard["_id"]
            host = shard["host"]
            shards[shard_id] = host
            # Количество реплик в шарде: host имеет вид "rs1/host1:port,host2:port,host3:port"
            if "/" in host:
                members_str = host.split("/", 1)[1]
                replicas_count[shard_id] = len(members_str.split(","))
            else:
                replicas_count[shard_id] = 1
        # Количество документов в каждом шарде по коллекциям
        shard_documents_count = {}
        for collection_name in collection_names:
            per_shard = await _get_shard_documents_count(collection_name)
            if per_shard:
                shard_documents_count[collection_name] = per_shard
    ...
    if replicas_count is not None:
        response["replicas_count"] = replicas_count
    if shard_documents_count is not None:
        response["shard_documents_count"] = shard_documents_count
    return response
...
async def list_users(collection_name: str):
    ...
    Кешируется в Redis: второй и последующие запросы выполняются <100мс.
    """
    time.sleep(1)
    collection = db.get_collection(collection_name)
